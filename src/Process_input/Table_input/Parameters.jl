...
"""
function process_parameters(parameters_file::CSV.File;
                            custom_parameter_values::Union{Nothing, Dict} = nothing)::ParametersInfo
    ...
    for i in eachindex(estimate)
        ...
        elseif parameters_file[:parameterScale][i] == "lin"
            ...
            error_str = "Parameter scale " * parameters_file[:parameterScale][i] *
                        "not supported. Only log10, log and lin are supported in the Parameters PEtab file under the paramter_scale column."
    ...
    return ParametersInfo(nominal_value, lower_bounds, upper_bounds, parameter_id,
                          paramter_scale, estimate, n_parameters_esimtate)
end
...
function process_priors(θ_indices::ParameterIndices, parameters_file::CSV.File)::PriorInfo
    ...
    for θ_name in θ_indices.θ_names
        which_parameter = findfirst(x -> x == string(θ_name),
                                    string.(parameters_file[:parameterId]))
        ...
        prior_parameters = parse.(Float64,
                                  split(parameters_file[which_parameter][:objectivePriorParameters],
                                        ";"))
