...
"""
function process_measurements(measurements_file::CSV.File,
                              observablesFile::CSV.File)::MeasurementsInfo
    ...
    noise_parameters::Vector{Union{String, Float64}} = Vector{Union{String, Float64}}(undef,
                                                                                      n_measurements)
    ...
    if :observableTransformation ∈ observablesFile.names
        for i in 1:n_measurements
            i_row = findfirst(x -> x == string(observable_Iid[i]),
                              observablesFile[:observableId])
            measurement_transformation[i] = Symbol(observablesFile[:observableTransformation][i_row])
        end
    end
    measurementT::Vector{Float64} = [transform_measurement_or_h(measurement[i],
                                                                measurement_transformation[i])
                                     for i in eachindex(measurement)]

    return MeasurementsInfo(measurement, measurementT, simulated_values, chi2_values,
                            residuals, measurement_transformation, time,
                            observable_Iid, pre_equilibration_condition_id,
                            simulation_condition_id, noise_parameters,
