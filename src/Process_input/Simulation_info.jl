...
function process_simulationinfo(petab_model::PEtabModel,
    ...
    for i in eachindex(measurement_info.pre_equilibration_condition_id)
        ...
        if _pre_equilibration_condition_id == :None
            measurement_info.simulation_condition_id[i] ∈ experimental_condition_id &&
                continue
            pre_equilibration_condition_id = vcat(pre_equilibration_condition_id, :None)
            simulation_condition_id = vcat(simulation_condition_id,
                                           measurement_info.simulation_condition_id[i])
            experimental_condition_id = vcat(experimental_condition_id,
                                             measurement_info.simulation_condition_id[i])
            continue
        end

        # For cases with no steady-state simulations
        _experimental_condition_id = Symbol(string(measurement_info.pre_equilibration_condition_id[i]) *
                                            string(measurement_info.simulation_condition_id[i]))
        if _experimental_condition_id ∉ experimental_condition_id
            pre_equilibration_condition_id = vcat(pre_equilibration_condition_id,
                                                  measurement_info.pre_equilibration_condition_id[i])
            simulation_condition_id = vcat(simulation_condition_id,
                                           measurement_info.simulation_condition_id[i])
            experimental_condition_id = vcat(experimental_condition_id,
                                             _experimental_condition_id)
            continue
        end
    end

    has_pre_equilibration_condition_id::Bool = all(pre_equilibration_condition_id .== :None) ?
                                               false : true
    ...
    ode_sols_pre_equlibrium::Dict{Symbol, Union{Nothing, ODESolution, SciMLBase.NonlinearSolution}} = Dict{Symbol,
                                                                                                           ODESolution}()
    ode_sols_derivatives::Dict{Symbol, Union{Nothing, ODESolution}} = Dict{Symbol,
                                                                           ODESolution}()
    for i in eachindex(experimental_condition_id)
        ...
        if pre_equilibration_condition_id[i] != :None &&
           pre_equilibration_condition_id[i] ∉ keys(ode_sols_pre_equlibrium)
    ...
    _tmax = Tuple(compute_tmax(pre_equilibration_condition_id[i],
                               simulation_condition_id[i], measurement_info)
                  for i in eachindex(pre_equilibration_condition_id))
    tmax::Dict{Symbol, Float64} = Dict([experimental_condition_id[i] => _tmax[i]
                                        for i in eachindex(_tmax)])

    # Precompute which time-points we have observed data at for experimental_condition_id (used in saveat for ODE solution)
    _time_observed = Tuple(compute_time_observed(pre_equilibration_condition_id[i],
                                                 simulation_condition_id[i],
                                                 measurement_info)
                           for i in eachindex(pre_equilibration_condition_id))
    time_observed::Dict{Symbol, Vector{Float64}} = Dict([experimental_condition_id[i] => _time_observed[i]
                                                         for i in eachindex(_time_observed)])

    # Precompute indices in measurement_info (i_measurement) for each experimental_condition_id
    _i_measurements_observed = Tuple(_compute_time_indices(pre_equilibration_condition_id[i],
                                                           simulation_condition_id[i],
                                                           measurement_info)
                                     for i in eachindex(pre_equilibration_condition_id))
    i_measurements_observed::Dict{Symbol, Vector{Int64}} = Dict([experimental_condition_id[i] => _i_measurements_observed[i]
                                                                 for i in eachindex(_i_measurements_observed)])
    ...
    i_time_ode_sol::Vector{Int64} = compute_time_index_ode_sol(pre_equilibration_condition_id,
    ...
    time_position_ode_sol::Dict{Symbol, UnitRange{Int64}} = get_time_position_ode_sol(experimental_condition_id,
                                                                                      time_observed)
    ...
    _i_per_time_point = Tuple(compute_time_indices(pre_equilibration_condition_id[i],
                                                   simulation_condition_id[i],
                                                   measurement_info)
                              for i in eachindex(pre_equilibration_condition_id))
    i_per_time_point::Dict{Symbol, Vector{Vector{Int64}}} = Dict([(experimental_condition_id[i],
                                                                   _i_per_time_point[i])
                                                                  for i in eachindex(_i_per_time_point)])
...
function _compute_time_indices(pre_equilibration_condition_id::Symbol,
                               simulation_condition_id::Symbol,
                               measurement_info::MeasurementsInfo)::Vector{Int64}
    i_timepoints = findall(i -> pre_equilibration_condition_id ==
                                measurement_info.pre_equilibration_condition_id[i] &&
                               simulation_condition_id ==
                               measurement_info.simulation_condition_id[i],
                           eachindex(measurement_info.time))
    return i_timepoints
end

function compute_time_indices(pre_equilibration_condition_id::Symbol,
                              simulation_condition_id::Symbol,
                              measurement_info::MeasurementsInfo)::Vector{Vector{Int64}}
    _i_timepoints = _compute_time_indices(pre_equilibration_condition_id,
                                          simulation_condition_id, measurement_info)
...
function compute_tmax(pre_equilibration_condition_id::Symbol,
                      simulation_condition_id::Symbol,
                      measurement_info::MeasurementsInfo)::Float64
    i_timepoints = _compute_time_indices(pre_equilibration_condition_id,
                                         simulation_condition_id, measurement_info)
    return Float64(maximum(measurement_info.time[i_timepoints]))
end

function compute_time_observed(pre_equilibration_condition_id::Symbol,
                               simulation_condition_id::Symbol,
                               measurement_info::MeasurementsInfo)::Vector{Float64}
    i_timepoints = _compute_time_indices(pre_equilibration_condition_id,
                                         simulation_condition_id, measurement_info)
...
function compute_time_index_ode_sol(pre_equilibration_condition_id::Vector{Symbol},
    ...
    for i in eachindex(simulation_condition_id)
        i_timepoints = _compute_time_indices(pre_equilibration_condition_id[i],
                                             simulation_condition_id[i], measurement_info)
