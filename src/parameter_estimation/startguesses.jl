...
- `sampling_method = LatinHypercubeSample()`: Method for sampling a diverse (spread out) set
   ...
   [QuasiMonteCarlo](https://github.com/SciML/QuasiMonteCarlo.jl) is allowed, but
   `LatinHypercubeSample` (default) is recommended as it usually performs well.
- `sample_prior::Bool = true`: Whether to sample random parameter values from the
   prior distribution if a parameter has an initialization prior.
- `allow_inf::Bool = true`: Whether to return parameter vectors for which the likelihood
   cannot be computed (typically happens because the `ODEProblem` cannot be solved). Since
   it often only makes sense to use starting points with a computable likelihood for
   parameter estimation, it typically does not make sense to change this option.
...
```
```julia
# Generate 10 initial guesses with Sobol sampling
using QuasiMonteCarlo: SobolSample
xstart = get_startguesses(prob, 10; sampling_method=SobolSample())
```
"""
