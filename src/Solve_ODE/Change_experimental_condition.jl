...
function _change_simulation_condition!(p_ode_problem::AbstractVector,
    ...
    if compute_forward_sensitivites == true
        S_t0::Matrix{Float64} = Matrix{Float64}(undef, (n_model_states, length(p_ode_problem)))
...
function _change_simulation_condition(p_ode_problem::AbstractVector,
    ...
    # Constant parameters
    _p_ode_problem = [i ∈ map_condition_id.i_ode_constant_parameters ? map_condition_id.constant_parameters[i_constant_param(i)] : p_ode_problem[i] for i in eachindex(p_ode_problem)]

    # Parameters to estimate
    __p_ode_problem = [i ∈ map_condition_id.i_ode_problem_θ_dynamic ? θ_dynamic[i_parameters_est(i)] : _p_ode_problem[i] for i in eachindex(_p_ode_problem)]
