...
Setup the [Fides](https://github.com/fides-dev/fides) box-constrained Newton-trust region
...
Fides is a Newton-trust region optimizer for box-constrained optmization problems. More
information on the algorithm can be found in [1]. Fides particularly excels when the full
Hessian is too computationally expensive to compute, but a Gauss-Newton Hessian
approximation can be computed (for more details see the documentation). In addition to
supporting user Hessians via the `PEtabODEProblem`, it supports several Hessian
approximation methods. Aa more extensive description than above see the Fides
[documentation](https://github.com/fides-dev/fides).
...
function Fides(hessian_method::Union{Nothing, Symbol}; verbose::Bool = false)
    if isnothing(hessian_method)
        return Fides(hessian_method, verbose)
    end
    allowed_methods = [nothing, :BB, :BFGS, :BG, :Broyden, :DFB, :FX, :SR1, :SSM, :TSSM]
    if !(hessian_method in allowed_methods)
        throw(PEtabInputError("$(hessian_method) is not a valid Hessian method for Fides \
