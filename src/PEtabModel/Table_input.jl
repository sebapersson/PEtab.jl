function PEtabModel(path_yaml::String;
    ...
    if !isfile(path_model_jl_file) || build_julia_files == true
        ...
        verbose == true && build_julia_files &&
            print(" By user option rebuilds Julia ODE model ...")
        verbose == true && !build_julia_files &&
    ...
    if !isfile(path_u0_h_sigma) || build_julia_files == true
        verbose == true && printstyled("[ Info:", color = 123, bold = true)
        verbose == true && !isfile(path_u0_h_sigma) &&
            print(" Building u0, h and σ file as it does not exist ...")
        verbose == true && isfile(path_u0_h_sigma) &&
            print(" By user option rebuilds u0, h and σ file ...")
        b_build = @elapsed h_str, u0!_str, u0_str, σ_str = create_σ_h_u0_file(model_name,
                                                                              path_yaml,
                                                                              dir_julia,
                                                                              ode_system,
                                                                              parameter_map,
    ...
    if !isfile(path_D_h_sd) || build_julia_files == true
        ...
        b_build = @elapsed ∂h∂u_str, ∂h∂p_str, ∂σ∂u_str, ∂σ∂p_str = create_derivative_σ_h_file(model_name,
                                                                                               path_yaml,
                                                                                               dir_julia,
                                                                                               ode_system,
                                                                                               parameter_map,
...
function get_function_str(file_path::AbstractString, n_functions::Int64;
                          as_str::Bool = false)::Vector{String}
    f_start, f_end = zeros(Int64, n_functions), zeros(Int64, n_functions)
    i_function = 1
    in_function::Bool = false
    if as_str == false
        n_lines = open(file_path, "r") do f
            countlines(f)
        end
    else
...
function add_parameters_condition_dependent_u0!(model_SBML::SBMLImporter.ModelSBML,
    ...
    which_species = (condition_variables[i_start:end])[findall(x -> x ∈ specie_names,
                                                               condition_variables[i_start:end])]
    ...
    for specie in which_species
        ...
        _parameter = SBMLImporter.ParameterSBML(_name, true, _value, "", false, false,
                                                false)
    ...
    rate_rule_parameters = filter(x -> x != "",
                                  [p.rate_rule ? p.name : ""
                                   for p in values(model_SBML.parameters)])
    which_parameters = (condition_variables[i_start:end])[findall(x -> x ∈
                                                                       rate_rule_parameters,
                                                                  condition_variables[i_start:end])]
    ...
    for specie in vcat(which_species, which_parameters)
        for row in conditions_df[!, Symbol(specie)]
            ...
            elseif row ∈ parameters_df[!, :parameterId]
                ...
                throw(PEtabFileError("The condition table value $row_value does not
