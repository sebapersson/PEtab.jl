function forwarddiff_gradient_chunks(f::F, Δx, x, ::ForwardDiff.Chunk{C};
                                     n_forward_passes = nothing) where {F, C}
    ...
    _n_forward_passes = isnothing(n_forward_passes) ? Int64(ceil(length(x) / C)) :
                        n_forward_passes
...
function forwarddiff_jacobian_chunks(f!::F, y, J, x, ::ForwardDiff.Chunk{C};
    ...
    for i in 1:_n_forward_passes
        ...
        if i_end ≤ (N + 1)
            ...
            ForwardDiff.seed!(xdual, x, i_start, seeds, lastchunksize)
            f!(ForwardDiff.seed!(ydual, y), xdual)
            ForwardDiff.extract_jacobian_chunk!(Nothing, Δx_reshaped, ydual, i_start,
                                                lastchunksize)
        end
    end
end
