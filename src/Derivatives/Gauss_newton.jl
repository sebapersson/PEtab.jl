function compute_jacobian_residuals_θ_dynamic!(jacobian::Union{Matrix{Float64}, SubArray},
    ...
    if reuse_sensitivities == false
        ...
        success = solve_sensitivites(ode_problem, simulation_info, θ_indices, petab_model, :ForwardDiff, θ_dynamicT,
    ...
    for i in eachindex(simulation_info.experimental_condition_id)
        ...
        compute_jacobian_residuals_condition!(jacobian, sol, petab_ODE_cache, θ_dynamicT, θ_sdT, θ_observableT,  
            θ_non_dynamicT, experimental_condition_id, simulation_condition_id, simulation_info, petab_model, θ_indices,
            measurement_info, parameter_info)
    end
end


function compute_jacobian_residuals_condition!(jacobian::AbstractMatrix,
                                               sol::ODESolution,
                                               petab_ODE_cache::PEtabODEProblemCache,
                                               θ_dynamic::Vector{Float64},
                                               θ_sd::Vector{Float64},
    ...
    iθ_experimental_condition = vcat(θ_indices.map_ode_problem.iθ_dynamic, map_condition_id.iθ_dynamic)
    ...
    for i in eachindex(time_observed)
        ...
        for i_measurement in i_per_time_point[i]
            ...
            adjust_gradient_θ_Transformed!((@view jacobian[:, i_measurement]), _gradient,
                                                 ∂G∂p, θ_dynamic, θ_indices, simulation_condition_id, autodiff_sensitivites=true)
...
function compute_residuals_not_solve_ode!(residuals::AbstractVector,
                                          θ_sd::AbstractVector,
                                          ...
                                          parameter_info::ParametersInfo,
                                          petab_ODE_cache::PEtabODEProblemCache;
                                          exp_id_solve::Vector{Symbol} = [:all])::AbstractVector
    ...
    for experimental_condition_id in simulation_info.experimental_condition_id
        ...
        sucess = compute_residuals_condition!(residuals, ode_sol, θ_sdT, θ_observableT,  θ_non_dynamicT,
                                              petab_model, experimental_condition_id, simulation_info, θ_indices, 
                                              measurement_info, parameter_info, petab_ODE_cache)
        if sucess == false
            residuals .= Inf
            break
        end
...
function compute_residuals_condition!(residuals::AbstractVector,
                                      ode_sol::ODESolution,
                                      θ_sd::AbstractVector,
                                      θ_observable::AbstractVector,
                                      θ_non_dynamic::AbstractVector,
                                      ...
                                      measurement_info::MeasurementsInfo,
                                      parameter_info::ParametersInfo,
                                      petab_ODE_cache::PEtabODEProblemCache)::Bool
    ...
    for i_measurement in simulation_info.i_measurements[experimental_condition_id]
        ...
        u .= dual_to_float.((@view ode_sol[:, simulation_info.i_time_ode_sol[i_measurement]]))
        p .= dual_to_float.(ode_sol.prob.p)
        hT = computehT(u, t, p, θ_observable,  θ_non_dynamic, petab_model, i_measurement, measurement_info, θ_indices, parameter_info)
        ...
        if measurement_info.measurement_transformation[i_measurement] == :lin
            residuals[i_measurement] = (hT - measurement_info.measurement[i_measurement]) / σ
        elseif measurement_info.measurement_transformation[i_measurement] == :log10
            residuals[i_measurement] = (hT - measurement_info.measurementT[i_measurement]) / σ
        elseif measurement_info.measurement_transformation[i_measurement] == :log
            residuals[i_measurement] = (hT - measurement_info.measurementT[i_measurement]) / σ
        else
            @error "Transformation ", measurement_info.measurement_transformation[i_measurement], " not yet supported."
        end
    end
    return true
end
