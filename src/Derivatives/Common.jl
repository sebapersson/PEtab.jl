function get_index_parameters_not_ODE(θ_indices::ParameterIndices)::Tuple

    θ_observable_names = θ_indices.θ_observable_names
    θ_sd_names = θ_indices.θ_sd_names
    θ_non_dynamic_names = θ_indices.θ_non_dynamic_names
    iθ_not_ode_names = θ_indices.θ_not_odeNames

    iθ_sd = [findfirst(x -> x == θ_sd_names[i], iθ_not_ode_names) for i in eachindex(θ_sd_names)]
    iθ_observable = [findfirst(x -> x == θ_observable_names[i],  iθ_not_ode_names) for i in eachindex(θ_observable_names)]
    iθ_non_dynamic = [findfirst(x -> x ==  θ_non_dynamic_names[i],  iθ_not_ode_names) for i in eachindex(θ_non_dynamic_names)]
