...
function _PEtabModel(
    ...
    # Ensure array inputs for ML models are parsed similar to problems in standard format
    _parse_array_input!(ml_models, petab_tables)

    return _PEtabModel(
        sys, petab_tables, name, speciemap, parametermap, events, ml_models, verbose
    )
end
