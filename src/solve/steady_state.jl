function solve_pre_equilibrium!!(u_ss::T, u_t0::T, @nospecialize(oprob::ODEProblem),
                                @nospecialize(simulation_info::SimulationInfo),
                                osolver::ODESolver, ss_solver::SteadyStateSolver,
                                preeq_id::Symbol,float_tspan::Bool)::Union{ODESolution, SciMLBase.NonlinearSolution} where {T <: AbstractVector}
    if ss_solver.method === :Simulate
        sol = _solve_ss(oprob, simulation_info, osolver, ss_solver, preeq_id, preeq_id, Float64[], float_tspan)
...
function _solve_ss(@nospecialize(oprob::ODEProblem), @nospecialize(simulation_info::SimulationInfo), osolver::ODESolver, ss_solver::SteadyStateSolver, cid::Symbol, simid::Symbol, tsave::Vector{Float64}, float_tspan::Bool)::ODESolution
    ...
    _cbs = CallbackSet(cbs.discrete_callbacks..., cbs.continuous_callbacks..., ss_solver.callback_ss)
