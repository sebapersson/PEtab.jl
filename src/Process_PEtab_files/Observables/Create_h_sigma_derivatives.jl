...
function create∂h∂_function(model_name::String,
    ...
    model_state_str, p_ode_problem_str,  θ_non_dynamic_str = create_top_∂h∂_function(model_state_names, 
        p_ode_problem_names, θ_non_dynamic_names, observables_data)
    ...
    for i in eachindex(observable_ids)
        ...
        for iState in eachindex(model_state_names)
            if occursin(Regex("\\b" * model_state_names[iState] * "\\b"), julia_formula)
                ...
                if !isempty(observable_parameters) && enter_observable == true
                    u_observeble_str *= "\t\t" * observable_parameters * " = get_obs_sd_parameter(θ_observable, parameter_map)\n"
                ...
                _∂h∂ui = string(Symbolics.derivative(julia_formula_symbolic, ui_symbolic; simplify=true))
                ∂h∂ui = variables_to_array_index(_∂h∂ui, model_state_names, parameter_info, p_ode_problem_names,  θ_non_dynamic_names, p_ode_problem=true)
        ...
        for ip in eachindex(p_ode_problem_names)
            if occursin(Regex("\\b" * p_ode_problem_names[ip] * "\\b"), julia_formula)
                ...
                observable_parameters = get_observable_parameters(formula)
                if !isempty(observable_parameters) && enter_observable == true
                    p_observeble_str *= "\t\t" * observable_parameters * " = get_obs_sd_parameter(θ_observable, parameter_map)\n"
                    enter_observable = false
                end

                julia_formula_symbolic =  eval(Meta.parse(julia_formula))
                pi_symbolic = eval(Meta.parse(p_ode_problem_names[ip]))
                _∂h∂pi = string(Symbolics.derivative(julia_formula_symbolic, pi_symbolic; simplify=true))
                ∂h∂pi = variables_to_array_index(_∂h∂pi, model_state_names, parameter_info, p_ode_problem_names,  θ_non_dynamic_names, p_ode_problem=true)
    ...
    write(io1, "function compute_∂h∂u!(u, t::Real, p_ode_problem::AbstractVector, θ_observable::AbstractVector,
    ...
    write(io2, "function compute_∂h∂p!(u, t::Real, p_ode_problem::AbstractVector, θ_observable::AbstractVector,
                       θ_non_dynamic::AbstractVector, observableId::Symbol, parameter_map::θObsOrSdParameterMap, out) \n")
...
function create∂σ∂_function(model_name::String,
    ...
    for i in eachindex(observable_ids)
        ...
        for iState in eachindex(model_state_names)
            if occursin(Regex("\\b" * model_state_names[iState] * "\\b"), julia_formula)
                ...
                if !isempty(noise_parameters) && enter_observable == true
                    u_observeble_str *= "\t\t" * noise_parameters * " = get_obs_sd_parameter(θ_sd, parameter_map)\n"
                    enter_observable = false
                end

                julia_formula_symbolic =  eval(Meta.parse(julia_formula))
                ui_symbolic = eval(Meta.parse(model_state_names[iState]))
                _∂σ∂ui = string(Symbolics.derivative(julia_formula_symbolic, ui_symbolic; simplify=true))
                ∂σ∂ui = variables_to_array_index(_∂σ∂ui, model_state_names, parameter_info, p_ode_problem_names,  θ_non_dynamic_names, p_ode_problem=true)
        ...
        for ip in eachindex(p_ode_problem_names)
            if occursin(Regex("\\b" * p_ode_problem_names[ip] * "\\b"), julia_formula)
                ...
                noise_parameters = get_noise_parameters(formula)
                if !isempty(noise_parameters) && enter_observable == true
                    p_observeble_str *= "\t\t" * noise_parameters * " = get_obs_sd_parameter(θ_sd, parameter_map)\n"
                    enter_observable = false
                end

                julia_formula_symbolic =  eval(Meta.parse(julia_formula))
                pi_symbolic = eval(Meta.parse(p_ode_problem_names[ip]))
                _∂σ∂pi = string(Symbolics.derivative(julia_formula_symbolic, pi_symbolic; simplify=true))
                ∂σ∂pi = variables_to_array_index(_∂σ∂pi, model_state_names, parameter_info, p_ode_problem_names,  θ_non_dynamic_names, p_ode_problem=true)
    ...
    write(io1, "function compute_∂σ∂σu!(u, t::Real, θ_sd::AbstractVector, p_ode_problem::AbstractVector,  θ_non_dynamic::AbstractVector,
    ...
    write(io2, "function compute_∂σ∂σp!(u, t::Real, θ_sd::AbstractVector, p_ode_problem::AbstractVector,  θ_non_dynamic::AbstractVector,
                        parameter_info::ParametersInfo, observableId::Symbol, parameter_map::θObsOrSdParameterMap, out) \n")
    write(io2, p_observeble_str)
    write(io2, "end")
