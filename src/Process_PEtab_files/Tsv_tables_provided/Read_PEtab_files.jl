...
function read_petab_files(path_yaml::String)
    ...
    check_petab_files(experimental_conditions, measurements_data, parameters_data, observables_data)
...
function check_petab_files(experimental_conditions::CSV.File, measurements_data, parameters_data, observables_data)
    ...
    check_df_columns(experimental_conditions, "experimental_conditions", columns_check, allowed_types, required_columns)
    ...
    if !isempty(columns_check)
        ...
        check_df_columns(experimental_conditions, "experimental_conditions", columns_check, allowed_types, [])
    end

    # Check measurements_data
    columns_check =     ["observableId", "simulationConditionId", "measurement", "time", "preequilibrationConditionId", "observableParameters", "noiseParameters", "datasetId", "replicateId"]
    allowed_types = [string_types, string_types, number_types, number_types, string_types, string_number_types, string_number_types, string_number_types, string_number_types]
    required_columns = ["observableId", "simulationConditionId", "measurement", "time"]
    check_df_columns(measurements_data, "measurements_data", columns_check, allowed_types, required_columns)

    # Check parameters_data
    columns_check = ["parameterId", "parameterScale", "lowerBound", "upperBound", "nominalValue", "estimate", "parameterName", "initializationPriorType", "initializationPriorParameters", "objectivePriorType", "objectivePriorParameters"]
    ...
    allowed_types = [string_types, transformation_types, number_types, number_types, number_types, estimateTypes, string_types, priorParameterTypes, string_types, priorParameterTypes, string_types]
    required_columns = ["parameterId", "parameterScale", "lowerBound", "upperBound", "nominalValue", "estimate"]
    check_df_columns(parameters_data, "parameters_data", columns_check, allowed_types, required_columns)

    # Check observables_data
    columns_check = ["observableId", "observableFormula", "noiseFormula", "observableName", "observableTransformation", "noiseDistribution"]
    allowed_types = [string_types, string_types, string_number_types, string_types, transformation_types, distribution_types]
    required_columns = ["observableId", "observableFormula", "noiseFormula"]
    check_df_columns(observables_data, "observables_data", columns_check, allowed_types, required_columns)
...
function check_df_columns(df, df_name, columns_check, allowed_types, required_columns)
    ...
    for col_index in eachindex(columns_check)
        ...
        if (column_name in required_columns) && !(column_name in df.names)
            throw(PEtabFileError("Required column '" * column_name * "' missing in file '" * df_name * "'"))
            ...
            if column_name == "upperBound" || column_name == "lowerBound"
                for row in eachindex(df[column_name])
                    if df[row][column_name] === missing && df[row][:estimate] == 1
                        println("Warning : Required column " * column_name * " contains rows with missing values on row " * string(row) * ".")
                    end
                end
            else
        ...
        elseif (allowed_types_column isa Array{String, 1}) || (allowed_types_column isa Array{Int64, 1})
            check &= all(x->x in allowed_types_column, column_to_check)
        end

        if !check
            throw(PEtabFileError("Wrong DataType or value in file '" * string(df_name) * "' column '" * string(column_name) * "'. Must be: " * "[" * join(allowed_types, ", ") * "]" * "."))
        end
    end
end
