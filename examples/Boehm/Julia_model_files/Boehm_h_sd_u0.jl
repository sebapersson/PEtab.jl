...
function compute_h(u::AbstractVector, t::Real, p_ode_problem::AbstractVector, θ_observable::AbstractVector,
	...
	if observableId === :pSTAT5A_rel 
		return ( 100 * u[6] + 200 * u[2] * p_ode_problem[6] ) / ( u[6] + u[1] * p_ode_problem[6] + 2 * u[2] * p_ode_problem[6] ) 
	end

	if observableId === :pSTAT5B_rel 
		return - ( 100 * u[6] - 200 * u[8] * ( p_ode_problem[6] - 1 ) ) / ( ( u[5] * ( p_ode_problem[6] - 1 ) - u[6] ) + 2 * u[8] * ( p_ode_problem[6] - 1 ) ) 
	end

	if observableId === :rSTAT5A_rel 
		return ( 100 * u[6] + 100 * u[1] * p_ode_problem[6] + 200 * u[2] * p_ode_problem[6] ) / ( 2 * u[6] + u[1] * p_ode_problem[6] + 2 * u[2] * p_ode_problem[6] - u[5] * ( p_ode_problem[6] - 1 ) - 2 * u[8] * ( p_ode_problem[6] - 1 ) ) 
	end
