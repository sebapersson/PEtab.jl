...
function getODEModel_Brannmark()
    ...
    ### Derivatives ###
    eqs = [
        D(IRp) ~ +1.0 * (1 / cyt) * (cyt * IRins * k1c) -
                 1.0 * (1 / cyt) * (cyt * IRp * k1d) - 1.0 * (1 / cyt) * (cyt * IRp * k1g),
        D(IR) ~ -1.0 * (1 / cyt) * (cyt * (IR * k1aBasic + IR * insulin * k1a)) +
                1.0 * (1 / cyt) * (cyt * IRins * k1b) +
                1.0 * (1 / cyt) * (cyt * IRp * k1g) + 1.0 * (1 / cyt) * (cyt * IRi * k1r),
        D(IRins) ~ +1.0 * (1 / cyt) * (cyt * (IR * k1aBasic + IR * insulin * k1a)) -
                   1.0 * (1 / cyt) * (cyt * IRins * k1b) -
                   1.0 * (1 / cyt) * (cyt * IRins * k1c),
        D(IRiP) ~ +1.0 * (1 / cyt) * (cyt * IRp * k1d) -
                  1.0 * (1 / cyt) * (cyt * IRiP * (k1e + Xp * k1f / (Xp + 1))),
        D(IRS) ~ -1.0 * (1 / cyt) * (cyt * IRS * k21 * (IRp + IRiP * k22)) +
                 1.0 * (1 / cyt) * (cyt * IRSiP * km2),
        D(X) ~ -1.0 * (1 / cyt) * (cyt * IRSiP * X * k3) +
               1.0 * (1 / cyt) * (cyt * Xp * km3),
        D(IRi) ~ +1.0 * (1 / cyt) * (cyt * IRiP * (k1e + Xp * k1f / (Xp + 1))) -
                 1.0 * (1 / cyt) * (cyt * IRi * k1r),
        D(IRSiP) ~ +1.0 * (1 / cyt) * (cyt * IRS * k21 * (IRp + IRiP * k22)) -
                   1.0 * (1 / cyt) * (cyt * IRSiP * km2),
        D(Xp) ~ +1.0 * (1 / cyt) * (cyt * IRSiP * X * k3) -
                1.0 * (1 / cyt) * (cyt * Xp * km3),
        insulin ~ insulin_dose_1 * ((1 - insulin_bool1) * (0) + insulin_bool1 * (1)) +
                  insulin_dose_2 * ((1 - insulin_bool2) * (0) + insulin_bool2 * (1)),
    ]
