...
function compute_h(u::AbstractVector, t::Real, p_ode_problem::AbstractVector,
    ...
    if observableId === :observable_CIS_au
        observableParameter1_observable_CIS_au, observableParameter2_observable_CIS_au = get_obs_sd_parameter(θ_observable,
                                                                                                              parameter_map)
        return observableParameter1_observable_CIS_au +
               u[22] * observableParameter2_observable_CIS_au / p_ode_problem[27]
    end
    ...
    if observableId === :observable_SOCS3RNA_foldA
        observableParameter1_observable_SOCS3RNA_foldA = get_obs_sd_parameter(θ_observable,
                                                                              parameter_map)
        return u[5] * observableParameter1_observable_SOCS3RNA_foldA / p_ode_problem[26] + 1
    end
    ...
    if observableId === :observable_SOCS3RNA_foldB
        observableParameter1_observable_SOCS3RNA_foldB = get_obs_sd_parameter(θ_observable,
                                                                              parameter_map)
        return u[5] * observableParameter1_observable_SOCS3RNA_foldB / p_ode_problem[26] + 1
    end
    ...
    if observableId === :observable_SOCS3RNA_foldC
        observableParameter1_observable_SOCS3RNA_foldC = get_obs_sd_parameter(θ_observable,
                                                                              parameter_map)
        return u[5] * observableParameter1_observable_SOCS3RNA_foldC / p_ode_problem[26] + 1
    end
    ...
    if observableId === :observable_SOCS3_au
        observableParameter1_observable_SOCS3_au, observableParameter2_observable_SOCS3_au = get_obs_sd_parameter(θ_observable,
                                                                                                                  parameter_map)
        return observableParameter1_observable_SOCS3_au +
               u[14] * observableParameter2_observable_SOCS3_au / p_ode_problem[39]
    end
    ...
    if observableId === :observable_pEpoR_au
        observableParameter1_observable_pEpoR_au, observableParameter2_observable_pEpoR_au = get_obs_sd_parameter(θ_observable,
                                                                                                                  parameter_map)
        return observableParameter1_observable_pEpoR_au +
               observableParameter2_observable_pEpoR_au *
               (16 * u[20] + 16 * u[1] + 16 * u[21]) / p_ode_problem[34]
    end
    ...
    if observableId === :observable_pJAK2_au
        observableParameter1_observable_pJAK2_au, observableParameter2_observable_pJAK2_au = get_obs_sd_parameter(θ_observable,
                                                                                                                  parameter_map)
        return observableParameter1_observable_pJAK2_au +
               observableParameter2_observable_pJAK2_au *
               (2 * u[23] + 2 * u[20] + 2 * u[1] + 2 * u[21]) / p_ode_problem[34]
    end
    ...
    if observableId === :observable_pSTAT5_au
        observableParameter1_observable_pSTAT5_au, observableParameter2_observable_pSTAT5_au = get_obs_sd_parameter(θ_observable,
                                                                                                                    parameter_map)
        return observableParameter1_observable_pSTAT5_au +
               u[2] * observableParameter2_observable_pSTAT5_au / p_ode_problem[30]
    end
...
function compute_u0!(u0::AbstractVector, p_ode_problem::AbstractVector)
    ...
    SOCS3nRNA4 = 0.0
    SOCS3RNA = 0.0
    SHP1 = p_ode_problem[23] * (1 + p_ode_problem[28] * p_ode_problem[33])
    ...
    CISnRNA3 = 0.0
    CISnRNA4 = 0.0
    SOCS3 = p_ode_problem[39] * p_ode_problem[21] * p_ode_problem[3]
    CISnRNA5 = 0.0
    SOCS3nRNA5 = 0.0
    SOCS3nRNA3 = 0.0
    SHP1Act = 0.0
    ...
    u0 .= p1EpoRpJAK2, pSTAT5, EpoRJAK2_CIS, SOCS3nRNA4, SOCS3RNA, SHP1, STAT5, EpoRJAK2,
          CISnRNA1, SOCS3nRNA1, SOCS3nRNA2, CISnRNA3, CISnRNA4, SOCS3, CISnRNA5, SOCS3nRNA5,
          SOCS3nRNA3, SHP1Act, npSTAT5, p12EpoRpJAK2, p2EpoRpJAK2, CIS, EpoRpJAK2, CISnRNA2,
          CISRNA
end
...
function compute_u0(p_ode_problem::AbstractVector)::AbstractVector
    ...
    return [p1EpoRpJAK2, pSTAT5, EpoRJAK2_CIS, SOCS3nRNA4, SOCS3RNA, SHP1, STAT5, EpoRJAK2,
            CISnRNA1, SOCS3nRNA1, SOCS3nRNA2, CISnRNA3, CISnRNA4, SOCS3, CISnRNA5,
            SOCS3nRNA5, SOCS3nRNA3, SHP1Act, npSTAT5, p12EpoRpJAK2, p2EpoRpJAK2, CIS,
            EpoRpJAK2, CISnRNA2, CISRNA]
end
...
function compute_σ(u::AbstractVector, t::Real, θ_sd::AbstractVector,
    ...
    if observableId === :observable_SOCS3RNA_foldA
        noiseParameter1_observable_SOCS3RNA_foldA = get_obs_sd_parameter(θ_sd,
                                                                         parameter_map)
        return noiseParameter1_observable_SOCS3RNA_foldA
    end

    if observableId === :observable_SOCS3RNA_foldB
        noiseParameter1_observable_SOCS3RNA_foldB = get_obs_sd_parameter(θ_sd,
                                                                         parameter_map)
        return noiseParameter1_observable_SOCS3RNA_foldB
    end

    if observableId === :observable_SOCS3RNA_foldC
        noiseParameter1_observable_SOCS3RNA_foldC = get_obs_sd_parameter(θ_sd,
                                                                         parameter_map)
    ...
    if observableId === :observable_pSTAT5_au
        noiseParameter1_observable_pSTAT5_au, noiseParameter2_observable_pSTAT5_au = get_obs_sd_parameter(θ_sd,
                                                                                                          parameter_map)
