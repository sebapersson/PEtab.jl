function getODEModel_Crauste_CellSystems2017(foo)
    ...
    ### Derivatives ###
    eqs = [
    D(Naive) ~ -1.0 * ( 1 /default ) * ((Naive*Pathogen)*delta_NE)-1.0 * ( 1 /default ) * (Naive*mu_N),
    ...
    D(LateEffector) ~ -1.0 * ( 1 /default ) * (LateEffector*delta_LM)-1.0 * ( 1 /default ) * ((LateEffector^2)*mu_LL)-1.0 * ( 1 /default ) * ((EarlyEffector*LateEffector)*mu_LE)+1.0 * ( 1 /default ) * (EarlyEffector*delta_EL),
    D(EarlyEffector) ~ +1.0 * ( 1 /default ) * ((EarlyEffector*Pathogen)*rho_E)-1.0 * ( 1 /default ) * ((EarlyEffector^2)*mu_EE)+1.0 * ( 1 /default ) * ((Naive*Pathogen)*delta_NE)-1.0 * ( 1 /default ) * (EarlyEffector*delta_EL),
