...
GRAD_MODELS = Dict(
    ...
    :Beer_MolBioSystems2014 => (
        test = [:forward_AD, :forward_eqs], tol = 1.0e-1, odetol = 1.0e-8, split = true,
    ),
    :Bruno_JExpBot2016 => (
        test = [:forward_AD, :forward_eqs], tol = 1.0e-1, odetol = 1.0e-8, split = false,
    ),
    :Brannmark_JBC2010 => (
        test = [:forward_AD, :forward_eqs], tol = 2.0e-3, odetol = 1.0e-10, split = false,
    ),
    :Schwen_PONE2014 => (
        test = [:forward_AD, :forward_eqs], tol = 1.0e-3, odetol = 1.0e-8, split = false,
    )
)
...
function test_nllh(modelid::Symbol)::Nothing
    ...
    @test nllh ≈ nllh_ref + prior_ref atol = 1.0e-2
    return nothing
end
...
function test_grad(modelid::Symbol)::Nothing
    ...
    for grad_test in grads_test
        ...
        elseif grad_test == :adjoint
            g = _compute_grad(
                x, model, :Adjoint, osolver1; ss_solver = ss_solver,
                sensealg = InterpolatingAdjoint(autojacvec = ReverseDiffVJP(true))
            )
        else
            g = _compute_grad(
                x, model, :ForwardDiff, osolver1; ss_solver = ss_solver,
                split = split
            )
        end
