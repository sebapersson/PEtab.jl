...
pnn1 = Lux.initialparameters(rng, nn14_1)
nnmodels = Dict(:net1 => NNModel(nn14_1, inputs = [:input1, :input2], outputs = [:gamma]),
                :net2 => NNModel(nn14_2, inputs = [:alpha, :predator], outputs = [:net2_output1]))
