...
PEtab.set_ml_model_ps!(pnn, path_h5, nn27_frozen, :net1)
# Set frozen parameters
pnn_tmp = Lux.initialparameters(rng, nn27) |> ComponentArray |> f64
PEtab.set_ml_model_ps!(pnn_tmp, path_h5, nn27, :net1)
...
ml_models = Dict(:net1 => MLModel(nn27_frozen; st = st, static = true, inputs = [:net1_input1, :net1_input2], outputs = [:gamma]))
