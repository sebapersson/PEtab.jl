...
nnmodels = Dict(:net1 => NNModel(nn7, inputs = [:input1, :input2], outputs = [:gamma]))
...
for config in PROB_CONFIGS
    if (test_case == "007" && config.grad == :ForwardEquations &&
        config.split == false && config.sensealg == :ForwardDiff)
        continue
    end
