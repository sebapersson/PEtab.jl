...
ml_models = MLModels(
    MLModel(:net1, nn9_1, false; inputs = [:prey, :predator], outputs = [:net1_output1]),
    MLModel(:net2, nn9_2, false; inputs = [:alpha, :predator], outputs = [:net2_output1])
)
path_h5 = joinpath(dir_case, "net1_ps.hdf5")
pnn1 = Lux.initialparameters(rng, nn9_1) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn1, path_h5, nn9_1, :net1)
path_h5 = joinpath(dir_case, "net2_ps.hdf5")
pnn2 = Lux.initialparameters(rng, nn9_2) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn2, path_h5, nn9_2, :net2)
