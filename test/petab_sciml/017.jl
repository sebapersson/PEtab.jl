...
nnmodels = Dict(:net1 => NNModel(nn17; static = true, inputs = [:net1_input1, :net1_input2], outputs = [:gamma]))
path_h5 = joinpath(@__DIR__, "test_cases", "hybrid", test_case, "petab", "net1_ps.hdf5")
pnn = Lux.initialparameters(rng, nn17) |> ComponentArray |> f64
PEtab.set_ps_net!(pnn, path_h5, nn17)

function lv17!(du, u, p, t)
    prey, predator = u
...
uprob = ODEProblem(lv17!, u0, (0.0, 10.0), p_mechanistic)
...
p_net1 = PEtabNetParameter(:net1, false, pnn)
