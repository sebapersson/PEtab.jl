...
using PEtab, OrdinaryDiffEqRosenbrock, OrdinaryDiffEqTsit5, SciMLSensitivity, ForwardDiff,
    LinearAlgebra, CSV, DataFrames, Sundials, Test
...
function test_odesolver(model::PEtabModel, osolver::ODESolver)::Nothing
    ...
    for i in 1:5
        ...
        sols = solve_all_conditions(
            x, prob, osolver.solver; abstol = osolver.abstol,
...
function test_nllh_grad_hess(model::PEtabModel, osolver::ODESolver)::Nothing
    ...
    @test all(.≈(H, hess_ref; atol = 5.0e-3))
    H = _compute_hess(x, model, :ForwardDiff, osolver; split = true)
    @test all(.≈(H, hess_ref; atol = 5.0e-3))
    H = _compute_hess(x, model, :BlockForwardDiff, osolver)
    @test all(.≈(H[1:2, 1:2], hess_ref[1:2, 1:2]; atol = 5.0e-3))
    @test all(.≈(H[3:4, 3:4], hess_ref[3:4, 3:4]; atol = 5.0e-3))
...
@testset "ODE solver" begin
    test_odesolver(model, ODESolver(Rodas5P(), abstol = 1.0e-9, reltol = 1.0e-9))
end

@testset "nllh, grad, and hess" begin
    test_nllh_grad_hess(model, ODESolver(Rodas5P(), abstol = 1.0e-15, reltol = 1.0e-15))
end
...
model2 = PEtabModel(
    joinpath(@__DIR__, "analytic_solution", "Test_model2.yaml"),
...
@test nllh ≈ nllh_ref atol = 1.0e-5
@test all(.≈(grad, grad_ref; atol = 1.0e-6))
