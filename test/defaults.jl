...
@testset "Test default options" begin
    ...
    path = joinpath(
        @__DIR__, "published_models", "Bachmann_MSB2011", "Bachmann_MSB2011.yaml"
    )
    ...
    prob = PEtabODEProblem(
        model; odesolver = ODESolver(CVODE_BDF()), odesolver_gradient = ODESolver(QNDF())
    )
    ...
    path = joinpath(
        @__DIR__, "published_models", "Beer_MolBioSystems2014", "Beer_MolBioSystems2014.yaml"
    )
    model = PEtabModel(path)
    prob = PEtabODEProblem(model)
    @test prob.probinfo.gradient_method === :ForwardDiff
    ...
    path = joinpath(
        @__DIR__, "published_models", "Smith_BMCSystBiol2013", "Smith_BMCSystBiol2013.yaml"
    )
