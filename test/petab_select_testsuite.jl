...
using PEtab, Optim, Fides, PyCall, YAML, Test, OrdinaryDiffEqRosenbrock
...
@testset "PEtab-select" begin
    ...
    path_save = petab_select(path_yaml, Fides.CustomHessian(), gradient_method=:ForwardEquations, hessian_method=:GaussNewton, reuse_sensitivities=true, sensealg=:ForwardDiff, nmultistarts=10)
    expected_res = YAML.load_file(path_exepected)
    data_res = YAML.load_file(path_save)
    @test expected_res["criteria"]["NLLH"] ≈ data_res["criteria"]["NLLH"] atol=1e-3
    rm(path_save)
    ...
    path_save = petab_select(path_yaml, Fides.CustomHessian(), gradient_method=:ForwardEquations,
