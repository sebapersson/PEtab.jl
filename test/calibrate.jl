...
@testset "Generate startguesses" begin
    ...
    prob = PEtabODEProblem(
        model; odesolver = ODESolver(Rodas5P(); verbose = false),
    )
    ...
    @test mean([x.a0 for x in xstarts]) ≈ 1.5 atol = 1.0e-1
    @test std([x.a0 for x in xstarts]) ≈ 0.1 atol = 1.0e-2
    @test mean([x.b0 for x in xstarts]) ≈ 1.5 atol = 1.0e-1
    @test std([x.b0 for x in xstarts]) ≈ 0.001 atol = 1.0e-2
...
@testset "Calibrate single start" begin
    ...
    @test all(.≈(res3.xmin, prob.xnominal_transformed, atol = 1.0e-2))
    # Testing Ipopt.jl
    res4 = calibrate(
    ...
    @test all(.≈(res4.xmin, prob.xnominal_transformed, atol = 1.0e-2))
    @test all(.≈(res5.xmin, prob.xnominal_transformed, atol = 1.0e-2))
    ...
    @test all(.≈(res6.u, prob.xnominal_transformed, atol = 1.0e-2))
    ...
    @test all(.≈(res7.xmin, prob.xnominal_transformed, atol = 1.0e-2))
    @test all(.≈(res8.xmin, prob.xnominal_transformed, atol = 1.0e-2))
end
...
@testset "Calibrate multi-start Fides" begin
    ...
    res1 = calibrate_multistart(
        prob, Optim.IPNewton(), 10; save_trace = true,
        dirsave = dirsave
    )
