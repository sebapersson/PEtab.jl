...
PEtab._set_ml_model_ps!(pnn, path_h5, nn28_frozen, :net1)
# Set frozen parameters
pnn_tmp = Lux.initialparameters(rng, nn28) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn_tmp, path_h5, nn28, :net1)
...
ml_models = MLModel(
    :net1, nn28_frozen, false; st = st, inputs = [:prey, :predator], outputs = [:net1_output1]
) |> MLModels
