...
pnn = Lux.initialparameters(rng, nn33) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn, path_h5, nn33, :net1)
...
# Test throws if specified layer does not exist
pest[end] = PEtabMLParameter(
    :net1; value = pnn, prior = Normal(0.0, 1.0), priors = ["layer_1" => Normal(0.0, 2.0)]
)
