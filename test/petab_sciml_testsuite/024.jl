...
PEtab._set_ml_model_ps!(pnn, path_h5, nn24_frozen, :net1)
# Set frozen parameters
pnn_tmp = Lux.initialparameters(rng, nn24) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn_tmp, path_h5, nn24, :net1)
...
ml_models = MLModel(
    :net1, nn24_frozen, true; st = st, inputs = [:net1_input1, :net1_input2],
    outputs = [:gamma]
) |> MLModels
