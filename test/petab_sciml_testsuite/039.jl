test_case = "039"
dir_case = joinpath(@__DIR__, "test_cases", "sciml_problem_import", test_case, "petab")

nn39 = @compact(
    layer1 = Dense(4, 5, Lux.relu),
    layer2 = Dense(5, 10, Lux.relu),
    layer3 = Dense(10, 1)
) do (x1, x2)
    x = vcat(x1, x2)
    embed = layer1(x)
    embed = layer2(embed)
    out = layer3(embed)
    @return out
end

ml_model = MLModel(
    :net6, nn39, false; inputs = ([:prey], [:net6_input]), outputs = [:net6_output1]
)
path_h5 = joinpath(dir_case, "net6_ps.hdf5")
pnn = Lux.initialparameters(rng, nn39) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn, path_h5, nn39, :net6)

function lv39!(du, u, p, t)
    prey, predator = u
    @unpack alpha, delta, beta, gamma = p
    du[1] = alpha * prey - beta * prey * predator # prey
    du[2] = gamma * predator * prey - delta * predator # predator
    return nothing
end

u0 = ComponentArray(prey = 0.44249296, predator = 4.6280594)
p_mechanistic = ComponentArray(alpha = 1.3, delta = 1.8, beta = 0.9, gamma = 0.8)
uprob = ODEProblem(lv39!, u0, (0.0, 10.0), p_mechanistic)

# Setup the PEtabModel as usual
# (algebraic expressions for initial values, specie-map overrides, will have to test later)
pest = [
    PEtabParameter(:alpha; scale = :lin, lb = 0.0, ub = 15.0, value = 1.3),
    PEtabParameter(:beta; scale = :lin, lb = 0.0, ub = 15.0, value = 0.9),
    PEtabParameter(:delta; scale = :lin, lb = 0.0, ub = 15.0, value = 1.8),
    PEtabParameter(:gamma; scale = :lin, lb = 0.0, ub = 15.0, value = 0.8),
    PEtabMLParameter(:net6; value = pnn),
]

observables = [
    PEtabObservable(:prey_o, "net6_output1 - 0.9 + prey", 0.05),
    PEtabObservable(:predator_o, :predator, 0.05),
]

conditions = [
    PEtabCondition(:e1, :net6_input => [1.0, 2.0, 3.0]),
    PEtabCondition(:e2, :net6_input => [3.0, 2.0, 1.0]),
]

path_m = joinpath(dir_case, "measurements.tsv")
measurements = CSV.read(path_m, DataFrame)
rename!(measurements, "experimentId" => "simulation_id")

model = PEtabModel(
    uprob, observables, measurements, pest; ml_models = ml_model,
    simulation_conditions = conditions
)
petab_prob = PEtabODEProblem(
    model; odesolver = ode_solver, gradient_method = :ForwardDiff,
    split_over_conditions = true
)
test_hybrid(test_case, petab_prob)
