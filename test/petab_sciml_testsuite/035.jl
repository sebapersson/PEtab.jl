...
pnn = Lux.initialparameters(rng, nn35) |> ComponentArray |> f64
PEtab._set_ml_model_ps!(pnn, path_h5, nn35, :net1)

function lv35!(du, u, p, t, ml_models)
    prey, predator = u
    @unpack alpha, delta, beta = p

    net1 = ml_models[:net1]
    du_nn, st = net1.lux_model([prey + (alpha - 1.3), predator], p[:net1], net1.st)
    net1.st = st
...
uprob = UDEProblem(lv35!, u0, (0.0, 10.0), p_mechanistic, ml_model)
