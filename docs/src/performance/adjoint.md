...
we use a published model (the Bachmann model [bachmann2011division](@cite)), available in
the PEtab standard format (see [Importing PEtab problems](@ref import_petab_problem)). The
PEtab files can be downloaded from
[here](https://github.com/sebapersson/PEtab.jl/tree/main/docs/src/assets/bachmann). Given
the problem YAML file, the model can be imported as:
...
2. `sensealg`: Adjoint algorithm and its Vector–Jacobian product (VJP) backend. PEtab.jl
   ...
   SciMLSensitivity. A key choice for these is the VJP backend; in practice `MooncakeVJP()`
   often performs well, and `ReverseDiffVJP(true)` is a good fallback.
...
are set via `abstol_adj`/`reltol_adj` in `ODESolver` and apply only to the adjoint solve. In
practice, relaxing the adjoint tolerances (relative to the default `1e-8`) can improve
...
In this setup `InterpolatingAdjoint` is faster (this can vary across machines). The VJP
backend can also matter:
...
@printf("Runtime MooncakeVJP(): %.1fs\n", b1)
@printf("Runtime ReverseDiffVJP(true): %.1fs\n", b2)
nothing # hide
```

Here `ReverseDiffVJP(true)` is faster (this can vary across machines). Since the best
combination is problem-dependent, benchmarking different `sensealg` and VJP options is
recommended for large models.

Finally, even when `gradient_method = :Adjoint` is fastest, `:ForwardDiff` is preferable if
it is not substantially slower. Adjoint gradients are more likely to fail (an additional
difficult backward adjoint solve is required), and forward-mode methods often produce more
accurate gradients.
