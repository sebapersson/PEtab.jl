...
A tutorial on creating a valid PEtab problem can be found in the PEtab
[documentation](https://petab.readthedocs.io/en/latest/), and the official PEtab Python
[linter](https://github.com/PEtab-dev/PEtab/tree/main) can be used to check format
correctness. A large set of ready-to-use PEtab problems is available in the PEtab benchmark
...
for downstream tasks such as parameter estimation Bayesian inference.

## Exporting a PEtab problem

After tasks like parameter estimation, a PEtab problem can be exported in the standard
format using `export_petab`. For example, run multi-start optimization:
...
Then export the best parameters in `ms_res` to a target directory:
...
!!! note "Export requires PEtab standard format input" `export_petab` currently only
supports problems that were imported from the PEtab standard format (YAML + tables).
...
`write_to_file = true`; files are written to `dir_yaml/Julia_model_files/`.
