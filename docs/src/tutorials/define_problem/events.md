...
Events/callbacks apply discrete changes during ODE simulation by updating states and/or
parameters. They are often used to model experimental interventions, such as dosing or a
media change. In PEtab.jl, events are specified with `PEtabEvent`:
...
State-triggered events fire when the `PEtabEvent` event condition transitions from `false`
to `true`. For example, to set `S => 1.0` when the state `S` reaches `0.2`:
...
If a model has multiple events, pass them as a `Vector{PEtabEvent}`. For example, assume
`event1` trigger when `S < 0.2` setting `S => S + 2`, and `event2` trigger at `t = 1.5`
setting `c1 => 2.0`:
