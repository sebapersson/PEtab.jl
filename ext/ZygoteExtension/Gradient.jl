...
function compute_gradient_zygote!(gradient::Vector{Float64},
    ...
    compute_gradient_zygote_θ_dynamic! = (x) -> _compute_cost_zygote(x, θ_sd, θ_observable,  θ_non_dynamic, ode_problem,
                                                                     petab_model, simulation_info, θ_indices, measurement_info,
                                                                     parameter_info, solve_ode_condition)
    gradient[θ_indices.iθ_dynamic] .= Zygote.gradient(compute_gradient_zygote_θ_dynamic!, θ_dynamic)[1]
    ...
    @views ReverseDiff.gradient!(gradient[iθ_not_ode], compute_cost_θ_not_ODE, θ_est[iθ_not_ode])
end
